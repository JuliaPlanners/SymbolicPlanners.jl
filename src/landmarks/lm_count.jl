...
function LMCount(lm_graph::LandmarkGraph, p_graph::PlanningGraph)
    return LMCount(lm_graph, LandmarkStatusManager(lm_graph, p_graph), nothing)
end
...
function compute(h::LMCount,
    ...
    future = get_future_landmarks(h.lm_status_manager)
    past = get_past_landmarks(h.lm_status_manager)
    curr_true = get_curr_true_landmarks(h.lm_status_manager)
    ...
    k = length(setdiff(future_past, curr_true))
