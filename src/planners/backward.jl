export BackwardAStarPlanner

"Heuristic-guided backward A* search."
@kwdef struct BackwardAStarPlanner <: Planner
    heuristic::Heuristic = GoalCountHeuristic(:backward)
    h_mult::Real = 1
    max_nodes::Real = Inf
end

set_max_resource(planner::BackwardAStarPlanner, val) =
    @set planner.max_nodes = val

"Backward A* search for a plan."
function call(planner::BackwardAStarPlanner,
